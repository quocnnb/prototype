...
- **Breadcrumb** (`.crumb`): ancestors and separators never shrink (`flex:0 0 auto`); the current-page item (`.bd`) shrinks and ellipsizes (`text-overflow`) so a long outlet name never overflows its container / the hero pill on narrow screens.
...
- **Social:** El Grande group social is **Instagram** (`instagram.com/elgrande_hk`). The standalone Instagram icons were removed from both the footer and the Contact page, so the group handle is not currently linked anywhere in the UI (re-add where desired). Per-outlet intro socials: Grappa's QRE, Cadillac, Mostaccioli, Happy Valley have Facebook + Instagram; Mo Bros Pizza, Mickey B (Happy Valley) and Gissons have none.
